...
class Play:
    def __init__(self, gym_env, base_logging_dir, user_id, should_log=True, fps=60, zoom=3, idle_threshold=1000, dump_frequency=300):
        if not hasattr(gym_env, "ale"):
            raise ValueError("only supports AtariEnv")
        ...
        game_name = self.env.gym_env.unwrapped.spec.id
        self.logging_dir = os.path.join(base_logging_dir, game_name, str(user_id))
...
if __name__ == "__main__":
    ...
    controller = Play(env, args.log_folder, args.user_id, should_log=not args.no_log, zoom=3)
    controller.play()
